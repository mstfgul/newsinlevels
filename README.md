...
Dört köşe var, hepsi aynı seviye/dil mantığını paylaşır:
...
- **Stories** — Project Gutenberg klasikleri (Oscar Wilde, Conan Doyle, Andersen…) günlük
  bölümler hâlinde tefrika edilir.
...
- **`pipeline/fetch-story.mjs`** — bir klasiğin (curated liste) planını çıkarır, her gün
  bir bölüm üretir; ilerleyiş `data/story-state.json`'da tutulur.
...
- **`pipeline/leveler.mjs`** — paylaşılan seviyeleme çağrısı (news/art/story/history) ve
  kategori sınıflandırıcı.
- **`.github/workflows/daily-*.yml`** — dört günlük pipeline (05:00 news, 05:30 art,
  05:45 story, 06:00 history; hepsi cron + elle). Her biri commit'ten önce `git pull
...
- **`.github/workflows/backfill.yml`** — elle tetiklenir; tüm eski içeriğe (haber, tablo,
  hikâye, tarih) eksik bir dili ve eksik kategorileri ekler:
