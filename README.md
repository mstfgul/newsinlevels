...
- **`pipeline/leveler.mjs`** — paylaşılan seviyeleme çağrısı (news/art/history) ve
  kategori sınıflandırıcı.
- **`.github/workflows/daily-*.yml`** — üç günlük pipeline (05:00 news, 05:30 art,
  06:00 history; hepsi cron + elle). Her biri commit'ten önce `git pull --rebase` yapar;
  Vercel push'u görüp siteyi otomatik yeniden yayınlar.
- **`.github/workflows/backfill.yml`** — elle tetiklenir; tüm eski içeriğe (haber, tablo,
  tarih) eksik bir dili ve eksik kategorileri ekler:
